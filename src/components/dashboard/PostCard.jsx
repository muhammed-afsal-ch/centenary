...
const PostCard = ({ caption, thumbnail, postId, createdAt, naseehath, video, videoheight, videowidth, imageheight, imagewidth }) => {
  ...
  useEffect(() => {
    if (thumbnail && thumbnail.length > 0 && thumbnail[0] && naseehath !== 'textonly' && !video) {
      console.log(`Loading image for post ${postId}:`, thumbnail[0]);
      const img = new window.Image();
      img.src = thumbnail[0];
      img.onload = () => {
        console.log(`Image loaded successfully for post ${postId}:`, { width: imagewidth || img.width, height: imageheight || img.height });
    ...
    } else if (video && naseehath === 'naseehath') {
      // Use video dimensions if available
      setImageDimensions({ width: videowidth || 640, height: videoheight || 360 });
    } else {
      console.log(`No thumbnail or video for post ${postId}, treating as text-only`);
    }
  }, [thumbnail, postId, naseehath, video, imagewidth, imageheight, videowidth, videoheight]);
  ...
  return (
    <div className="flex flex-col items-center px-4 mb-8 border border-white pt-4 rounded-lg bg-gray-50">
      ...
      <div className="mt-4 w-full relative rounded-lg overflow-hidden">
        ...
        ) : video && naseehath === "naseehath" && typeof video === 'string' && video.startsWith('http') ? (
          !isPlaying ? (
            <button
            ...
            >
              <img
                ...
                alt="Video Thumbnail"
                style={{ width: adjustedWidth, height: videoheight, objectFit: 'cover' }}
            ...
            <video
              ref={videoRef}
              src={video}
              style={{ width: adjustedWidth, height: videoheight, borderRadius: 8 }}
              controls
        ...
        {!isLogged && naseehath !== "textonly" && !video && (
          <button
