...
export async function POST(req) {
  ...
  try {
    ...
    const fullUrl = `${baseUrl}api/updates/delete`;
    ...
    const response = await fetch(fullUrl, {
      ...
      body: JSON.stringify({ postId: updateId }),
    ...
    return new Response(JSON.stringify({ message: 'Update deleted successfully!', ...data }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error in deleteupdates API:', error);
