...
export default function AddUpdate() {
  ...
  const handleSubmit = async (e) => {
    ...
    form.images.forEach((image, index) => {
      formData.append('images', image.file, `updates_${index}.jpg`);
    });
    ...
    try {
      ...
      setMessage('Update uploaded successfully!');
  ...
  return (
    <div className="max-w-xl w-full mx-auto bg-white p-6 rounded-xl shadow-xl space-y-6">
      <h2 className="text-2xl font-bold text-center text-green-700">Add New Update</h2>

      {/* Error Message */}
      {error && (
        <div className="p-4 bg-red-50 text-red-800 rounded-lg shadow-sm flex justify-between items-center">
          <span>{error}</span>
          <button
            onClick={() => setError('')}
      ...
      {/* Success Message */}
      {message && (
        <div className="p-4 bg-green-50 text-green-800 rounded-lg shadow-sm flex justify-between items-center">
          <span>{message}</span>
          <button
            onClick={() => setMessage('')}
            className="text-sm font-semibold hover:underline"
